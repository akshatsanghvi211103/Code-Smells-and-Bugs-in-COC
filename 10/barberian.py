...
## Refactoring implemented: Removed Duplicated Code

def check_health(obj,count,name,a,b):
    if obj[0] <= 0:
        if name == "canon 1":
            config.C1DEAD = True
        disappear.delete_ele(a, b, name)
        count[0] += 1
...
class barberian:
    ...
    def attack(self, a, b, c1,c2,c3,h1,h2,h3,h4,h5,th,count):
        ...
        for i, j in directions:
            ...
            if b[adjacentY][adjacentX].split(" ")[0] == 'canon':
                index = int(b[adjacentY][adjacentX].split(" ")[1])
                c[index-1][0] -= self.damage
                check_health(c[index-1],count,b[adjacentY][adjacentX],a,b)
                # if c[index-1][0] <= 0:
                #     if(index == 1):
                #         config.C1DEAD = True
                #     disappear.delete_ele(a, b, b[adjacentY][adjacentX])
                #     count[0]+=1

            elif b[adjacentY][adjacentX].split(" ")[0] == 'Hut':
                index = int(b[adjacentY][adjacentX].split(" ")[1])
                h[index-1][0] -= self.damage
                check_health(h[index-1],count,b[adjacentY][adjacentX],a,b)
                # if h[index-1][0] <= 0:
                #     disappear.delete_ele(a, b, b[adjacentY][adjacentX])
                #     count[0]+=1
                ...
                th[0] -= self.damage
                check_health(th,count,b[adjacentY][adjacentX],a,b)
                # if th[0] <= 0:
                #     disappear.delete_ele(a, b, b[adjacentY][adjacentX])
                #     count[0]+=1
